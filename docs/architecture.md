...
- Logging helpers live in `pkg/common/logging` and are used by binaries for consistent prefixes/UTC timestamps.
- CLI registry lives in `pkg/cli` with initial `show` commands; intended to be exposed via embedded SSH/HTTP transports in management plane.
